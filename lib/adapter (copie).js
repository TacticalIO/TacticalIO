...
var TIOAdapter = function(args, cb) {
   ...
   * @api private
   */
  var initHSGPIO32 = function(id, cb) {
  	debug('Board init: ' + id);
  	cb();
  }
   ...
   * @api private
   */
  var initHSAIO12 = function(id, cb) {
  	debug('Board init: ' + id);
  	cb();
  }	
   ...
   * @api private
   */
  var initHSFSKCOM = function(id, cb) {
  	debug('Board init: ' + id);
  	cb();
  }
			...
			var csmuxCb = function(pin) {
				return function(callback) {
					try {
						...
						debug('error when configuring pin %d: %s', pin, err);
						callback(err);
					}
				}
			}(csmuxPins[i]);
	...
	self.cs = function(args, cb) { 
		async.series([
			...
			function(callback) {
				...
				if (!err) {
					debug('CS ' + args.id + (args.enable ? ' enabled': ' disabled') 
						+ ' with mux = ' + muxMap[args.id]); 
				} else {
					debug('CS ' + args.id + ' set failed');
				}
				if (cb) cb(err);
			});
	}
	...
	self.writeCPUDigital = function(args, cb) {
		...
		try {
			...
			debug('error when writing CPU IO %s: %s', 
				args.name, err);
			cb(err);
		}
	}
	...
	self.readCPUDigital = function(args, cb) {
		...
		try {
			...
			debug('error when reading CPU IO %s: %s', 
				args.name, err);
			cb(err);
		}
	}
	...
	self.spiTransfer = function(args, cb) {
		spi.transfer(args.data, args.data.length, function(err, data) {
			if (!err) {
				debug('spi transfer done with length ' + data.length + ' bytes');
			} else {
				debug('spi transfer failed: ' + err);
			}
			if (cb) cb(err, data);
		});		
	}
	...
	for (var i = 0; i < io.length; i++) {
		...
		debug('Board configuration parse done for ' + io[i].id);
	...
	for (var i = 0; i < io.length; i++) {
		if (!io[i].spi_master) {
			csMap[io[i].id] = 'CS' + (io[i].cs + 1);
			muxMap[io[i].id] = io[i].mux;
    }
	}

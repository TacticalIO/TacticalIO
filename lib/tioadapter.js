...
var TIOAdapter = function(args, cb) {
  ...
  var initHSFSKCOM = function(spi_address, cb) {
	...
	self.configCPUPins = function(cb) {
		var setupArray = [];
		var cmuxPins = [29, 31, 32];
		for (var i = 0; i < 3; i++) {
			var cmuxCb = function(pin) {
				return function(callback) {
					gpio.setup(pin, gpio.DIR_OUT, callback);
				}
			}(cmuxPins[i]);
			setupArray.push(cmuxCb);
		}
		for (var cs in csMap) {
		...
		for (var name in IO) {
			if (IO[name].boardType == 'CPU-RPI') {
				var csCb = function(io) {
					return function(callback) {
						if (io.type == 'DI') {
							gpio.setup(io.channel, gpio.DIR_IN, callback);
						} else if (io.type == 'DO') {
							gpio.setup(io.channel, gpio.DIR_OUT, callback);
						}
					}
				}(IO[name]);
				setupArray.push(csCb);

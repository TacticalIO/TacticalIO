...
var LogApp = function(options) {
  ...
  if (options.log.destination == 'db') {
		MongoClient.connect(options.log.url, function(err, db) {
  		if (!err) {
  			console.log('[' + options.log + '] last message before switch to db: db connected');
  		} else {
  			console.error('[' + options.log + '] db connection error: ' + err);
  		}
  		mongodb = db;
		});
  }
  ...
  logServer.on('message', function (msg, rinfo) {
    ...
    if (options.log.destination == 'console') {
    	console.log.apply(console, message);
  		...
  		var level = message.slice(0, 5).replace(/[\[\]]/, '');
  		var info = message.slice(6);
		  mongodb.collection('logs').insert({
		  	level: level,
		  	message: message
		  }, function(err, result) {
		  	if (err) {
		  		console.error('[' + options.log + '] db log error');
		  	}
		  });
  	}
  });

...
function encodeFreq(buf, freq) {
	var ftw = (freq *  Math.pow(2, 48)) / 120000000;
	var byte0 = ftw % 255;
	ftw = ftw / 256;
	var byte1 = ftw % 255;
	ftw = ftw / 256;
	...
	ftw = ftw / 256;
	var byte5 = ftw % 255;
	buf.writeUInt8(parseInt(byte5), 2);
	buf.writeUInt8(parseInt(byte4), 3);
	buf.writeUInt8(parseInt(byte3), 4);
	buf.writeUInt8(parseInt(byte2), 5);
	buf.writeUInt8(parseInt(byte1), 6);
	buf.writeUInt8(parseInt(byte0), 7);
} 
...
var TIOAdapter = function(args) {
  ...
  var initHSFSKCOM = function(id) {
	...
	self.spiTransfer = function(args) {
		...
		if (args.id !== undefined && io[args.id].type != 'GPIO64') {
			...
			if(result[0] & 0x40) {
				debug('SPI comm error (Last command invalid) on card' + args.id);
			}
			/* check the last command incomplete status bit */
			if(result[0] & 0x80) {
				debug('SPI comm error (Last command incomplete) on card' + args.id);
			}
		}
   ...
   *		2 = Always low
	...
	self.writeFSK = function(args) {
		...
		var voltage = parseInt(args.voltage ? (args.voltage > 725 ? 725 : args.voltage) : 725);
		var gain = parseInt(voltage * (Math.pow(2, 12)-1) / 725);
		if (channel == 1) gain |= 0x80; // mark the voltage for channel 1

...
const FSK_5V_AMPLITUDE = 5/7.6*725;
const FSK_3V_AMPLITUDE = 3/7.6*725;
const FSK_1V_AMPLITUDE = 1/7.6*725;

var FSK_MAX_VOLTAGE = parseInt(FSK_MAX_AMPLITUDE * (Math.pow(2, 12)-1) / FSK_MAX_AMPLITUDE);
var FSK_5V_VOLTAGE = parseInt(FSK_5V_AMPLITUDE * (Math.pow(2, 12)-1) / FSK_MAX_AMPLITUDE);
var FSK_3V_VOLTAGE = parseInt(FSK_3V_AMPLITUDE * (Math.pow(2, 12)-1) / FSK_MAX_AMPLITUDE);
var FSK_1V_VOLTAGE = parseInt(FSK_1V_AMPLITUDE * (Math.pow(2, 12)-1) / FSK_MAX_AMPLITUDE);
...
var TIOAdapter = function(args) {
   ...
   *		2 = Always low
	...
	self.setDigitalFreq = function(args) {
		...
		var freq = (2*args.freq * Math.pow(2, 32)) / 100000000;
   ...
   * { name: ..., freq: ..., offset: ..., when: ... }
	...
	self.setDigitalFreq2ch = function(args) {
		debug('Digital (freq,offset) for pattern (both)');
		...
		if (args[0].when !== undefined && args[0].when != 0) {
			try {
				var when = parseInt((args[0].when - self.clockResetTime)/1.024);
				buf.writeInt32BE(when, 2);
			} catch(e) {
				debug('certainly value out of bounds: ' + when);
				buf.writeUInt32BE(0, 2);
		...
		/* Maximum frequency is set to 256kHz in the FPGA (hardware limit) 
		   any value higher than that will be set back to 256kHz */
		var freq = (2*args[0].freq * Math.pow(2, 32)) / 100000000;
		buf.writeUInt32BE(parseInt(freq), 6);
		var offset = (args[0].offset * Math.pow(2, 32)) / 360;
		buf.writeUInt32BE(parseInt(offset), 10);
		// second
		buf.writeUInt8(IO[args[1].name].patternId, 14);
		if (args[1].when !== undefined && args[1].when != 0) {
			try {
				var when = parseInt((args[1].when - self.clockResetTime)/1.024);
				buf.writeInt32BE(when, 15);
			} catch(e) {
				debug('certainly value out of bounds: ' + when);
				buf.writeUInt32BE(0, 15);
			}
		} else {
			buf.writeUInt32BE(0, 15);
		}
		/* Maximum frequency is set to 256kHz in the FPGA (hardware limit) 
		   any value higher than that will be set back to 256kHz */
		freq = (2*args[1].freq * Math.pow(2, 32)) / 100000000;
		buf.writeUInt32BE(parseInt(freq), 19);
		offset = (args[1].offset * Math.pow(2, 32)) / 360;

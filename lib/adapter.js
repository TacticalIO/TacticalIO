	...
	self.spiTransfer = function(args) {
		...
		if (args.id !== undefined && io[args.id].type != 'GPIO64') {
			...
			if(result[0] & 0x0F != 0x0A) {
				debug('SPI comm error (Invalid constant ID byte) : ' + args.id, spiLastReception);
			}
			/* check the reset status bit */
			if ((result[0] & 0x10) == 0) {
				debug('Board error (Board has not been reset since power-up) : ' + args.id, spiLastReception);
			}
			/* check the last command invalid status bit */
			if(result[0] & 0x40) {
				debug('SPI comm error (Last command invalid) on board ' + args.id, spiLastReception);
			}
			/* check the last command incomplete status bit */
			if(result[0] & 0x80) {
				debug('SPI comm error (Last command incomplete) on board ' + args.id, spiLastReception);
			}
		}

...
function encodeFreq(buf, freq) {
	var ftw = parseInt((freq *  POW2_48) / 120000000);
	var byte0 = ftw%255;
		...
		if (args.values !== undefined) {
  		buf.writeUInt16BE(b(args.values, 0, 16, 16), 1);
			...
			for (var pos = 0; pos < 16; pos++) {
				...
				debug('TIO.DO' + paddedCounter + ' = ' + values[pos]);
				outCounter--;	
			}
  	...
  	for (var i = 0; i < 4; i++) {
	  	...
	  	data.writeUInt8(0, 2);
			...
			for (var pos = 7; pos >= 0; pos--) {
				...
				if (IO['DI' + paddedCounter].value != old ) {
					self.emit('DI' + paddedCounter, IO['DI' + paddedCounter].value ? 0 : 1);
					ds.set('TIO.DI' + paddedCounter, IO['DI' + paddedCounter].value ? 0 : 1);
				}
				counter--;	
			} 
  	}

...
var TIOAdapter = function(args) {
  ...
  var initHSFSKCOM = function(id) {
	...
	self.writeAnalog = function(args) {
		...
		self.cs({ id: IO[args.name].id, enable: 1 });
		self.spiTransfer( { id: IO[args.name].id, data: buf });		
		self.cs({ id: IO[args.name].id, enable: 0 });
	}
	...
	self.readAIO12Analog8 = function(args) {
		...
		var resultCallback = function() {
			var rBuf = new Buffer(18);
			rBuf.fill(0);
			rBuf.writeUInt8(0x06, 0);		
			self.cs({ id: IO[args.name].id, enable: 1 });
			var result = self.spiTransfer( { id: args.id, data: rBuf } );
			self.cs({ id: IO[args.name].id, enable: 0 });
			if (result[1] & 0x01) {

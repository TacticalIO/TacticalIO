...
function encodeFreq(buf, freq) {
	var ftw = parseInt((freq *  POW2_12) / 120000000);
	var byte0 = ftw%255;
...
const FSK_MAX_AMPLITUDE = 5760;

const POW2_32 = Math.pow(2, 32);
const POW2_12 = Math.pow(2, 12);
const POW2_48 = Math.pow(2, 48);
...
var TIOAdapter = function(args) {
	...
	if (process.arch == 'arm') {
		rpio = require('rpio');
		rpio.spiClockSpeedDivider(32); //32);
		rpio.spiDataMode(0);
  ...
  var initGPIO64 = function(id) {
  	...
  	var tbuf = new Buffer([0x41, 0x0A]);
  	debug('read ICON', self.spiTransfer({ id: id, data : tbuf }));
		...
		for (var i = 0; i < 4; i++) {
			...
			var result = self.spiTransfer({ id: id, data : buf });
			self.cs({ id: id, enable: false });
			debug(id + ' CS[' + GPIO64_CS[i].toString(2) + '] ', buf, result);
		}
  ...
  var initHSFSKCOM = function(id) {
  	/* init DDS */
  	var buf = new Buffer([0x04,0x07,0x10,0x04,0x02,0x21]);
  	self.spiTransfer({ id: id, data: buf });
	...
	self.configCPUPins = function() {
		for (var name in IO) {
			var ioc = IO[name];
			if (ioc.boardType == 'CPU-RPI') {
				try {
					if (ioc.type == 'DI') {
						ioc.handle = new Gpio(ioc.channel, 'in');
					} else if (ioc.type == 'DO' || ioc.type == 'CSMUX' ||ioc.type == 'CS') {
						ioc.handle = new Gpio(ioc.channel, 'out');
					}
				} catch(err) {
					debug('error when configuring CPU IO pin %d: %s', ioc.channel, err);
				}
			}
		}
	 ...
	 */
	self.reset = function() {
		var css = io['CPU'].CS.channels;
		for (var i = 1; i <= 8; i++) {
			IO['CS'+i].handle.writeSync(1);
		}
	...
	var spiLastReception, spiLastEmission;
	...
	self.spiTransfer = function(args) {
		...
		if (0 && args.id !== undefined && io[args.id].type != 'GPIO64') {
			// 0: fake test purposes: TO BE REMOVED
			/* check constant ID bit in the status register */
			if(result[0] & 0x0F != 0x0A) {
				debug('SPI comm error (Invalid constant ID byte) : ' + args.id, 'last TX', spiLastEmission, 'last RX', spiLastReception, 'RX', result);
			}
			/* check the reset status bit */
			if ((result[0] & 0x10) == 0) {
				debug('Board error (Board has not been reset since power-up) : ' + args.id, 'last TX', spiLastEmission, 'last RX', spiLastReception, 'RX', result);
			}
			/* check the last command invalid status bit */
			if(result[0] & 0x40) {
				debug('SPI comm error (Last command invalid) on board ' + args.id, 'last TX', spiLastEmission, 'last RX', spiLastReception, 'RX', result);
			}
			/* check the last command incomplete status bit */
			if(result[0] & 0x80) {
				debug('SPI comm error (Last command incomplete) on board ' + args.id, 'last TX', spiLastEmission, 'last RX', spiLastReception, 'RX', result);
			}
		}
		spiLastReception = result;
		spiLastEmission = args.data;
		return result;
	}
	...
	self.writeHSGPIO32Digital16 = function(args) {
		...
		for (var pos = 15; pos >= 0; pos--) {
			...
			IO['DI' + paddedCounter].value = (values & ( 1 << pos )) ? 0 : 1;
   ...
   *		2 = Always low
	...
	self.readGPIO64Digital32 = function(args) {
  	...
  	for (var i = 0; i < 4; i++) {
	  	...
	  	data.writeUInt8(0, 2);
			...
			//response = self.spiTransfer({ data : data });
			//self.cs({ id: args.id, enable: false });
			debug('R64 [MCP ' + i + ']: ', response);
			values = values | (response.readUInt8(2) << (i*8));
			for (var pos = 7; pos >= 0; pos--) {
				var paddedCounter = String('00' + counter).slice(-2);
				var old = IO['DI' + paddedCounter].value;
	...
	self.readDigital = function(args) {		
		...
		if (IO[args.name].boardType == 'HS-GPIO32') {
			...
			var value = IO[args.name].handle.readSync() ? 0 : 1;
			ds.set('TIO.' + args.name, value);
			return value;
		}
	}
	...
	self.enableComTX = function(args) {
		...
		var buf = new Buffer([addr, 0x09, 0x00]);
		...
		buf = new Buffer([addr, 0x89, register]);
		self.spiTransfer( { id: IO[args.name].id, data: buf } );
	}
	...
	self.initCom = function(args) {
		...
		function initUARTGPIO(outlevel) {
			// Setup UART  GPIO dir
			var ubuf = new Buffer([addr, 0x98, 0x0f]);
		...
		var buf = new Buffer([addr, 0x9e, 0x18]);
		self.spiTransfer( { id: IO[args.name].id, data: buf } );

		buf = new Buffer([addr, 0x09, 0x00]);
		var readBuf = self.spiTransfer( { id: IO[args.name].id, data: buf } );
	 ...
	 * @param {string} name - output name as per configuration data
	 * @param {buffer} data - data to send packaged as Buffer
	 * @param {int} lastByteSize - number of bits in the last byte
	 * @param {number} voltage - voltage value
	 * @param {boolean} continuous - ask for continuous send
	 * @param {number} when - timestamp for delayed execution (absolute UTC value)
	 * @api public
	 */	
	self.writeFSK = function(args) {
		var channel = IO[args.name].channel;
		var buf = new Buffer(8 + args.data.length);
		buf.writeUInt8(0x07, 0);

 		var lastByteSize = 
			parseInt(args.lastByteSize ? (args.lastByteSize > 8 ? 8 : args.lastByteSize) : 8);

		if(args.continuous == 1) {
			lastByteSize += 0x10; /* Set bit 4 to have continous send mode selected 
															(this requires a stop command to stop sending FSK data) */
		}
		var voltage = parseInt(args.voltage ? (args.voltage > FSK_MAX_AMPLITUDE ? FSK_MAX_AMPLITUDE : args.voltage) : FSK_MAX_AMPLITUDE);
		//args.voltage = parseInt(args.voltage ? args.voltage/7.6*FSK_MAX_AMPLITUDE : FSK_5V_AMPLITUDE);
		//var voltage = args.voltage > FSK_MAX_AMPLITUDE ? FSK_MAX_AMPLITUDE : args.voltage;
		var gain = parseInt(voltage*(POW2_12 - 1) / FSK_MAX_AMPLITUDE);
		...
		if (args.when !== undefined && args.when != 0) {
 		...
 		buf.writeUInt8(lastByteSize, 7);
		args.data.copy(buf, 8);
		self.spiTransfer( { id: IO[args.name].id, data: buf } );
	}

	/* 
	 * Stops FSK channels
	 * @param {string} id - FSK board id
	 * @param {buffer} data - data to send packaged as Buffer
	 * @param {number} when - timestamp for delayed execution (absolute UTC value)
	 * @api public
	 */	
	self.stopFSK = function(args) {
		var buf = new Buffer(5);
	...
	self.initFSK = function(args) {
		...
		var brfreq = parseInt(args.bitrate*POW2_32/100000000);
	...
	self.setDigitalFreq = function(args) {
		...
		var freq = (2*args.freq * POW2_32) / 100000000;
		buf.writeUInt32BE(parseInt(freq), 6);
		var offset = (args.offset * POW2_32) / 360;
	 ...
	 * Digital output is configured for a given frequency and offset. If frequency == 0, 
	 * then the signal is standby (not generated) 
   * @param {array} setup - array of objects two channels, as per single setup data  
   * { name: ..., freq: ..., offset: ..., when: ... }
	 * @api public
	 */
	self.setDigitalFreq2ch = function(args) {
		var buf = new Buffer(27), 
			when = 0;
		buf.writeUInt8(0x09, 0);
		buf.writeUInt8(IO[args[0].name].patternId, 1);
		if (args[0].when !== undefined && args[0].when != 0) {
			try {
				when = parseInt((args[0].when - self.clockResetTime)/1.024);
				buf.writeInt32BE(when, 2);
			} catch(e) {
				debug('certainly value out of bounds: ' + when);
				when = 0;
		...
		/* Maximum frequency is set to 256kHz in the FPGA (hardware limit) 
		   any value higher than that will be set back to 256kHz */
		var freq = (2*args[0].freq * POW2_32) / 100000000;
		buf.writeUInt32BE(parseInt(freq), 6);
		var offset = (args[0].offset * POW2_32) / 360;
		buf.writeUInt32BE(parseInt(offset), 10);
		// second
		buf.writeUInt8(IO[args[1].name].patternId, 14);
		buf.writeUInt32BE(when, 15);
		/* Maximum frequency is set to 256kHz in the FPGA (hardware limit) 
		   any value higher than that will be set back to 256kHz */
		freq = (2*args[1].freq * POW2_32) / 100000000;
		buf.writeUInt32BE(parseInt(freq), 19);
		offset = (args[1].offset * POW2_32) / 360;
		buf.writeUInt32BE(parseInt(offset), 23);
		self.spiTransfer( { id: IO[args[0].name].id, data: buf });
	}
	...
	for (var uid in io) {
		if (!io[uid].spi_master && io[uid].type != 'DUMMY') {

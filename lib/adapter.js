...
var TIOAdapter = function(args) {
	...
	self.reset = new Gpio(14, 'out'); 	
	/* Sync clock hard reset pin */
	self.syncReset = new Gpio(17, 'out'); 
  ...
  var initHSFSKCOM = function(id) {
	...
	/* 
	 * Returns board's internal id if FPGA board
	 * @param {string} id - CS id
	 * @return {int} - Internal ID
	 * @api public
	 */
	self.iid = function(args) {
		var buf = new Buffer(4);
		buf.fill(0);
		return self.spiTransfer({ id: args.id, data: buf });
	}
	...
	self.writeAnalog = function(args) {
		...
		buf.writeUInt32BE(parseInt((args.when - self.clockResetTime)*1.024), 1);
		buf.writeUInt8(channel, 5); // parseInt(channel,8)
		buf.writeUInt16BE(dac_value, 6);
	...
	self.hwSyncReset = function() {
		self.syncReset.writeSync(1); 			
		self.syncReset.writeSync(1); 	
		self.syncReset.writeSync(0); 
		self.syncReset.writeSync(0);
	}
